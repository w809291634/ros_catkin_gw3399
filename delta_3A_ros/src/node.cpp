...
int main(int argc, char * argv[])
{
    ...
    nh_private.param<int>("rotationl_speed", rotationl_speed, 10);
    if (rotationl_speed < 5) rotationl_speed = 5;
    if (rotationl_speed > 15) rotationl_speed = 15;
    printf("set rotionl speed %d\n", rotationl_speed);
...
//    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>(lidar_scan, 1000);
	...
	#if 1 
	retvalue = robotics_lidar.setLidarRotationlSpeed(rotationl_speed*100);
	if(retvalue == EXECUTE_SUCCESS )
    ...
    ROS_INFO("lidar node exit...\n");
    robotics_lidar.setLidarWorkMode(IDLE_MODE);
